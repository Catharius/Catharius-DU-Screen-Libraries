if not DonutChart then
    ...
    function DonutChart:new(posX, posY, circleDiameter, dataUnit)
        ...
        function self:draw(dataLuaArray)
            ...
            local smallFont = loadFont("Play", tonumber(string.format('%.0f', (self.circleDiameter * 3 / 100))))
            ...
            for i = 1, #firstRow do
                ...
                for k = 2, #dataLuaArray do
                    ...
                    if dataMin[1] > data or dataMin[1] < 0 then
                        dataMin[1] = data
                        dataMin[2] = dataLabelWidth
                        dataMin[3] = dataLabelHeight
                        dataMin[4] = dataLabel
                    end
                    if dataMax[1] < data or dataMax[1] < 0 then
                        dataMax[1] = data
                        dataMax[2] = dataLabelWidth
                        dataMax[3] = dataLabelHeight
                        dataMax[4] = dataLabel
                    end
                ...
                table.sort(leftLabelList, function(a, b)
                    return a.y < b.y
                end)
                ...
                lowerOccupiedY = -1
                table.sort(rightLabelList, function(a, b)
                    return a.y < b.y
                end)
                ...
                elseif self.currentCentralMenu == 3 then
                    ...
                    setNextTextAlign(labelLayer, AlignH_Center, AlignV_Baseline)
                    if dataMin[2] > circleRadius then
                        addText(labelLayer, smallFont, dataMin[4], self.x, self.y - dataMin[3])
                    else
                        addText(labelLayer, mediumFont, dataMin[4], self.x, self.y - dataMin[3])
                    end
                    setNextTextAlign(labelLayer, AlignH_Center, AlignV_Baseline)
                    local dataValue = tonumber(string.format('%.2f', dataMin[1]))
                    if string.len(dataValue) > 14 then
                        addText(labelLayer, smallFont, tonumber(string.format('%.2f', dataMin[1])), self.x, self.y + dataMin[3])
                    else
                        addText(labelLayer, mediumFont, tonumber(string.format('%.2f', dataMin[1])), self.x, self.y + dataMin[3])
                    end
                elseif self.currentCentralMenu == 4 then
                    setNextTextAlign(labelLayer, AlignH_Center, AlignV_Baseline)
                    addText(labelLayer, mediumFont, "MAX", self.x, mediumLabelY)
                    setNextTextAlign(labelLayer, AlignH_Center, AlignV_Baseline)
                    if dataMax[2] > circleRadius then
                        addText(labelLayer, smallFont, dataMax[4], self.x, self.y - dataMax[3])
                    else
                        addText(labelLayer, mediumFont, dataMax[4], self.x, self.y - dataMax[3])
                    end
                    setNextTextAlign(labelLayer, AlignH_Center, AlignV_Baseline)
                    local dataValue = tonumber(string.format('%.2f', dataMax[1]))
                    if string.len(dataValue) > 14 then
                        addText(labelLayer, smallFont, tonumber(string.format('%.2f', dataMax[1])), self.x, self.y + dataMax[3])
                    else
                        addText(labelLayer, mediumFont, tonumber(string.format('%.2f', dataMax[1])), self.x, self.y + dataMax[3])
                    end
                end
...
if not alreadyDeclared then
    ...
    -- posX and posY are coordinates of the center, dataUnit is the value type label (Use the string  "L" for fluids for example)
